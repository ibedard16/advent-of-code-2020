...
class CubeField {
        ...
        for (let xIndex = cube.x - 1; xIndex <= cube.x + 1; ++xIndex) {
            for (let yIndex = cube.y - 1; yIndex <= cube.y + 1; ++yIndex) {
                for (let zIndex = cube.z - 1; zIndex <= cube.z + 1; ++zIndex) {
                    for (let wIndex = cube.w - 1; wIndex <= cube.w + 1; ++wIndex) {
                        if (xIndex === cube.x && yIndex === cube.y && zIndex === cube.z && wIndex === cube.w) {
                            continue;
                        }
                        
                        neighbors.push(this.getCubeWithIndex(xIndex,yIndex,zIndex,wIndex));
                    }
                }
            }
        }
    ...
    getCubeWithIndex(x: number, y: number, z: number, w: number): Cube {
        let cube = this.cubes.find(cube => cube.x === x && cube.y === y && cube.z === z && cube.w === w);
    ...
    doTick(): CubeField {
        ...
        dyingCubes.forEach(dyingCube => 
            nextTick.getCubeWithIndex(dyingCube.x, dyingCube.y, dyingCube.z, dyingCube.w).isActive = false
        );
        
        const birthingCubes = this.cubes.filter(cube => !cube.isActive && cube.activeNeighborsCount === 3);
        birthingCubes.forEach(birthingCube => 
            nextTick.getCubeWithIndex(birthingCube.x, birthingCube.y, birthingCube.z, birthingCube.w).isActive = true
        );
